...
DSPDLL_API INT DSpotter_SetEnableNBest(HANDLE hDSpotter, INT bEnable);
DSPDLL_API INT DSpotter_GetNBestScore(HANDLE hDSpotter, INT pnCmdIdx[], INT pnScore[], INT nMaxNBest);
...
// Return: Success or error code
DSPDLL_API INT DSpotter_SetResponseTime(HANDLE hCSpotter, INT nResponseTime);

// Purpose: Set Cmd response time
// hCSpotter(IN): a handle of the recognizer
// nCmdIdx(IN): the command ID. It is 0 based
// nOffset(IN): response time
// Return: Success or error code
DSPDLL_API INT DSpotter_SetCmdResponseTime(HANDLE hCSpotter, INT nCmdIdx, INT nResponseTime);
